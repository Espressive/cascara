...
const parse = __webpack_require__(655);
const enoent = __webpack_require__(738);
...
const resolveCommand = __webpack_require__(534);
const escape = __webpack_require__(902);
const readShebang = __webpack_require__(964);
...
const pathKey = __webpack_require__(661)();
...
const crossSpawn = __webpack_require__(987);
...
const build_utils_1 = __importDefault(__webpack_require__(178));
const { getWriteableDirectory, download, glob, createLambda, shouldServe, debug, } = build_utils_1.default;
...
exports.build = async ({ workPath, files: originalFiles, entrypoint, meta = {}, config, }) => {
...
async function pipInstall(workPath, args) {
    ...
    debug(`Running "pip install --disable-pip-version-check --target ${target} --upgrade ${args.join(' ')}"...`);
    try {
        await execa_1.default(pipPath, [
            'install',
        ...
        console.log(`Failed to run "pip install --disable-pip-version-check --target ${target} --upgrade ${args.join(' ')}"...`);
        throw err;
    }
}
async function installRequirement({ dependency, workPath, meta, args = [], }) {
    if (meta.isDev && (await isInstalled(dependency, workPath))) {
        debug(`Skipping ${dependency} dependency installation, already installed in ${workPath}`);
        return;
    }
