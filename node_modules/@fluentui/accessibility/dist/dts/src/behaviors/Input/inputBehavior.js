"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.inputBehavior = void 0;
var tslib_1 = require("tslib");
var keyboard_key_1 = require("../../keyboard-key");
/**
 * @specification
 * Adds attribute 'aria-disabled=true' based on the property 'disabled' to 'input' slot.
 * Adds attribute 'aria-required=true' based on the property 'required' to 'input' slot.
 * Adds attribute 'aria-invalid=true' based on the property 'error' to 'input' slot.
 * Triggers 'clear' action with 'Escape' on 'input'.
 */
var inputBehavior = function (props) { return ({
    attributes: {
        input: tslib_1.__assign(tslib_1.__assign({ 'aria-disabled': props.disabled }, (props.required && { 'aria-required': true })), (props.error && { 'aria-invalid': true })),
    },
    keyActions: {
        input: {
            clear: {
                keyCombinations: [{ keyCode: keyboard_key_1.keyboardKey.Escape }],
            },
        },
    },
}); };
exports.inputBehavior = inputBehavior;
