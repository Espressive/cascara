...
var getReactFiberFromNode_1 = require("../getReactFiberFromNode");
...
function hasAutofocusProp(node) {
    ...
    if (isAutoFocusableElement) {
        return !!getReactFiberFromNode_1.getReactFiberFromNode(node).pendingProps.autoFocus;
    }
    return false;
}
...
function usePopperOptions(options, popperOriginalPositionRef) {
    var autoSize = options.autoSize, flipBoundary = options.flipBoundary, offset = options.offset, onStateUpdate = options.onStateUpdate, overflowBoundary = options.overflowBoundary, rtl = options.rtl, unstable_disableTether = options.unstable_disableTether, unstable_pinned = options.unstable_pinned;
    var placement = positioningHelper_1.getPlacement(options.align, options.position, options.rtl);
    var strategy = options.positionFixed ? 'fixed' : 'absolute';
    var handleStateUpdate = react_bindings_1.useEventCallback(function (_a) {
        var state = _a.state;
        if (onStateUpdate) {
            onStateUpdate(state);
        }
    ...
    var userModifiers = react_bindings_1.useDeepMemo(function () { return options.modifiers; }, options.modifiers);
    return React.useCallback(function (target, container, arrow) {
        var scrollParentElement = getScrollParent_1.getScrollParent(container);
        var hasScrollableElement = scrollParentElement
            ? scrollParentElement !== scrollParentElement.ownerDocument.body
            : false;
        var modifiers = tslib_1.__spreadArrays([
            /**
            ...
            flipBoundary && {
                ...
                options: {
                    altBoundary: true,
                    boundary: getBoundary_1.getBoundary(container, flipBoundary),
                },
            },
            overflowBoundary && {
                name: 'preventOverflow',
...
function usePopper(options) {
    ...
    var isFirstMount = react_bindings_1.useFirstMount();
    ...
    var handlePopperUpdate = react_bindings_1.useEventCallback(function () {
        var _a;
        (_a = popperInstanceRef.current) === null || _a === void 0 ? void 0 : _a.destroy();
        popperInstanceRef.current = null;
        var popperInstance = null;
        if (isBrowser_1.isBrowser() && enabled) {
    ...
    var targetRef = react_bindings_1.useCallbackRef(null, handlePopperUpdate, true);
    var containerRef = react_bindings_1.useCallbackRef(null, handlePopperUpdate, true);
    var arrowRef = react_bindings_1.useCallbackRef(null, handlePopperUpdate, true);
