...
describe('Menu', function () {
    commonTests_1.isConformant(Menu_1.Menu, { testPath: __filename, constructorName: 'Menu', autoControlledProps: ['activeIndex'] });
    ...
    describe('items', function () {
        ...
        it('should open on hover and keep open on click', function () {
            var items = getNestedItems();
            items[1].on = 'hover';
            var menu = utils_1.mountWithProvider(React.createElement(Menu_1.Menu, { items: items }));
        ...
        it('closes menu when item is clicked using spacebar', function () {
            ...
            menuItems.at(1).find('a').first().simulate('keydown', { keyCode: accessibility_1.SpacebarKey });
            expect(menuItems.at(1).at(0).find('a').first().getDOMNode().getAttribute('aria-expanded')).toBe('true');
            menuItems.at(1).at(0).find('a').first().simulate('keydown', { keyCode: accessibility_1.SpacebarKey });
        ...
        describe('variables', function () {
            function checkMergedVariables(menu, isFunction) {
                if (isFunction === void 0) { isFunction = false; }
                var menuVariables = menu.find('MenuItem').first().prop('variables');
                var dividerVariables = menu.find('MenuDivider').first().prop('variables');
                expect(isFunction ? menuVariables() : menuVariables).toEqual(expect.objectContaining({ a: 'menu', b: 'overwritten', c: 'item' }));
                expect(isFunction ? dividerVariables() : dividerVariables).toEqual(expect.objectContaining({ a: 'menu', b: 'overwrittenInDivider', c: 'divider' }));
            }
        ...
        describe('children', function () {
            it('should should select items', function () {
                var onActiveIndexChange = jest.fn();
                var wrapper = utils_1.mountWithProvider(React.createElement(Menu_1.Menu, { defaultActiveIndex: 0, onActiveIndexChange: onActiveIndexChange },
                    React.createElement(Menu_1.Menu.Item, { index: 0 },
                        React.createElement(Menu_1.Menu.ItemContent, null, "Editorials")),
                    React.createElement(Menu_1.Menu.Item, { index: 1 },
                        React.createElement(Menu_1.Menu.ItemContent, null, "Reviews")),
                    React.createElement(Menu_1.Menu.Item, { index: 2 },
                        React.createElement(Menu_1.Menu.ItemContent, null, "Upcoming Events"))));
