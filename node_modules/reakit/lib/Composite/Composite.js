...
var isIE11 = dom.canUseDOM && "msCrypto" in window;
...
function useKeyboardEventProxy(virtual, currentItem, htmlEventHandler) {
  var eventHandlerRef = hooks.useLiveRef(htmlEventHandler);
  return React.useCallback(function (event) {
    ...
    if (virtual && canProxyKeyboardEvent(event)) {
      ...
      if (currentElement) {
        if (!events.fireKeyboardEvent(currentElement, event.type, event)) {
...
function useActiveElementRef(elementRef) {
  ...
  React.useEffect(function () {
    var document = dom.getDocument(elementRef.current);
...
function useScheduleUserFocus(currentItem) {
  var currentItemRef = hooks.useLiveRef(currentItem);
...
var useComposite = createHook.createHook({
  ...
  useProps: function useProps(options, _ref) {
    ...
    var onFocusCaptureRef = hooks.useLiveRef(htmlOnFocusCapture);
      ...
      if (events.isSelfTarget(event) && nextActiveElementIsItem) {
        ...
        if (nextActiveElement === currentElement) {
          ...
          if (previousElementRef.current && previousElementRef.current !== nextActiveElement) {
            ...
            events.fireBlurEvent(previousElementRef.current, event);
          }
          ...
          events.fireBlurEvent(currentElement, event);
