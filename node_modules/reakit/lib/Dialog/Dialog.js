...
var events = require('reakit-utils/events');
var focus = require('reakit-utils/focus');
...
var MenuContext = require('../MenuContext-8c9dd060.js');
...
function useDisclosureRef(dialogRef, options) {
  ...
  React.useEffect(function () {
    ...
    var document = dom.getDocument(dialogRef.current);
  ...
  React.useEffect(function () {
    ...
    var onMouseDown = function onMouseDown(event) {
      ...
      if (!dom.isButton(element)) return;
...
function useFocusOnShow(dialogRef, nestedDialogs, options) {
  ...
  hooks.useUpdateEffect(function () {
    ...
    if (initialFocusRef !== null && initialFocusRef !== void 0 && initialFocusRef.current) {
      ...
      var tabbable = focus.getFirstTabbableIn(dialog, true);

      var isActive = function isActive() {
        return focus.hasFocusWithin(dialog);
      };

      if (tabbable) {
        focus.ensureFocus(tabbable, {
          preventScroll: true,
          isActive: isActive
        });
      } else {
...
function usePortalRef(dialogRef, options) {
  ...
  React.useEffect(function () {
    ...
    portalRef.current = dom.closest(dialog, Portal_Portal.Portal.__selector);
...
function useFocusTrap(dialogRef, visibleModals, options) {
  ...
  React.useEffect(function () {
    ...
    if (!beforeElement.current) {
      var document = dom.getDocument(portal);
  ...
  React.useEffect(function () {
    ...
    var handleFocus = function handleFocus(event) {
      ...
      var tabbable = isAfter ? focus.getFirstTabbableIn(dialog) : focus.getLastTabbableIn(dialog);
...
function hidByFocusingAnotherElement(dialogRef) {
  ...
  var activeElement = dom.getActiveElement(dialog);
  if (!activeElement) return false;
  if (dom.contains(dialog, activeElement)) return false;
  if (focus.isTabbable(activeElement)) return true;
...
function useFocusOnHide(dialogRef, disclosureRef, options) {
  ...
  hooks.useUpdateEffect(function () {
    ...
    if (finalFocusEl) {
      if (finalFocusEl.id) {
        var document = dom.getDocument(finalFocusEl);
        var compositeElement = document.querySelector("[aria-activedescendant='" + finalFocusEl.id + "']");

        if (compositeElement) {
          focus.ensureFocus(compositeElement);
          return;
        }
      }

      focus.ensureFocus(finalFocusEl);
      return;
    }
...
function useNestedDialogs(dialogRef, options) {
  ...
  var removeDialog = React.useCallback(function (ref) {
    ...
    setDialogs(function (prevDialogs) {
      return misc.removeItemFromArray(prevDialogs, ref);
    });
...
function dialogContains(target) {
  return function (dialogRef) {
    ...
    var document = dom.getDocument(dialog);
    var backdrop = document.querySelector("[data-dialog-ref=\"" + dialog.id + "\"]");

    if (backdrop) {
      return dom.contains(backdrop, target);
    }
...
function isDisclosure(target, disclosure) {
  return dom.contains(disclosure, target);
}

function isInDocument(target) {
  var document = dom.getDocument(target);

  if (target.tagName === "HTML") {
    return true;
  }

  return dom.contains(document.body, target);
}

function useEventListenerOutside(containerRef, disclosureRef, nestedDialogs, eventType, listener, shouldListen, capture) {
  var listenerRef = hooks.useLiveRef(listener);
  React.useEffect(function () {
    ...
    var document = dom.getDocument(containerRef.current);
...
function useHideOnClickOutside(dialogRef, disclosureRef, nestedDialogs, options) {
  ...
  useEventListenerOutside(dialogRef, disclosureRef, nestedDialogs, "focusin", function (event) {
    var document = dom.getDocument(dialogRef.current); // Fix for https://github.com/reakit/reakit/issues/619
...
function useFocusOnChildUnmount(dialogRef, options) {
  React.useEffect(function () {
    ...
    var observer = new MutationObserver(function (mutations) {
      ...
      var document = dom.getDocument(dialog);
      var activeElement = dom.getActiveElement(dialog); // We can check if the current focused element is the document body. On
      // IE 11, it's an empty object when the current document is in a frame or
      // iframe.

      if (activeElement === document.body || misc.isEmpty(activeElement)) {
        dialog.focus();
      }
    });
...
function isActualElement(element) {
  return element && element.tagName && element.tagName !== "HTML" && element !== dom.getDocument(element).body;
}
...
function useFocusOnBlur(dialogRef, options) {
  ...
  hooks.useSafeLayoutEffect(function () {
    ...
    if (!isActualElement(dom.getActiveElement(dialog))) {
  ...
  var onBlur = React.useCallback(function (event) {
    if (!options.visible) return;
    var nextActiveElement = events.getNextActiveElementOnBlur(event);
...
var useDialog = createHook.createHook({
  ...
  useProps: function useProps(options, _ref2) {
    ...
    var onKeyDownRef = hooks.useLiveRef(htmlOnKeyDown);
    var onBlurRef = hooks.useLiveRef(htmlOnBlur);
    ...
    return _rollupPluginBabelHelpers._objectSpread2({
      ref: hooks.useForkRef(dialog, htmlRef),
      role: "dialog",
