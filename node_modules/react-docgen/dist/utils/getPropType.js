...
function getEnumValues(path, importer) {
  ...
  path.get('elements').each(function (elementPath) {
    if (_astTypes.namedTypes.SpreadElement.check(elementPath.node)) {
      const value = (0, _resolveToValue.default)(elementPath.get('argument'), importer);
    ...
    const value = (0, _resolveToValue.default)(elementPath, importer);
    return values.push({
...
function getPropTypeOneOf(argumentPath, importer) {
  ...
  let value = (0, _resolveToValue.default)(argumentPath, importer);

  if (!_astTypes.namedTypes.ArrayExpression.check(value.node)) {
    value = (0, _resolveObjectKeysToArray.default)(value, importer) || (0, _resolveObjectValuesToArray.default)(value, importer);
...
function getPropTypeShapish(name, argumentPath, importer) {
  ...
  if (!_astTypes.namedTypes.ObjectExpression.check(argumentPath.node)) {
    argumentPath = (0, _resolveToValue.default)(argumentPath, importer);
  }
  ...
  if (_astTypes.namedTypes.ObjectExpression.check(argumentPath.node)) {
    ...
    argumentPath.get('properties').each(function (propertyPath) {
      ...
      const propertyName = (0, _getPropertyName.default)(propertyPath, importer);
...
function getPropTypeInstanceOf(argumentPath, importer) {
  return {
    name: 'instanceOf',
