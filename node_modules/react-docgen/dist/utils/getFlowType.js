...
function handleArrayTypeAnnotation(path, typeParams, importer) {
  return {
    name: 'Array',
    elements: [getFlowTypeWithResolvedTypes(path.get('elementType'), typeParams, importer)],
    raw: (0, _printValue.default)(path)
  };
}
...
function handleGenericTypeAnnotation(path, typeParams, importer) {
  ...
  const resolvedPath = typeParams && typeParams[type.name] || (0, _resolveToValue.default)(path.get('id'), importer);
  ...
  path.get('properties').each(param => {
    if (_astTypes.namedTypes.ObjectTypeProperty.check(param.node)) {
      type.signature.properties.push({
        ...
        key: (0, _getPropertyName.default)(param, importer),
        value: getFlowTypeWithRequirements(param.get('value'), typeParams, importer)
      });
    }
  });
...
function handleUnionTypeAnnotation(path, typeParams, importer) {
  return {
    name: 'union',
    raw: (0, _printValue.default)(path),
    elements: path.get('types').map(subType => getFlowTypeWithResolvedTypes(subType, typeParams, importer))
  };
}
...
function handleFunctionTypeAnnotation(path, typeParams, importer) {
  ...
  path.get('params').each(param => {
    ...
    type.signature.arguments.push({
      ...
      type: typeAnnotation ? getFlowTypeWithResolvedTypes(typeAnnotation, typeParams, importer) : undefined
    });
  });
