...
function resolveHOC(path, importer) {
  ...
  if (_astTypes.namedTypes.CallExpression.check(node) && !(0, _isReactCreateClassCall.default)(path, importer) && !(0, _isReactForwardRefCall.default)(path, importer)) {
    if (node.arguments.length) {
      ...
      if (node.arguments.length > 1 && (_astTypes.namedTypes.Literal.check(inner.node) || _astTypes.namedTypes.ObjectExpression.check(inner.node) || _astTypes.namedTypes.ArrayExpression.check(inner.node) || _astTypes.namedTypes.SpreadElement.check(inner.node))) {
        return resolveHOC((0, _resolveToValue.default)(path.get('arguments', node.arguments.length - 1), importer), importer);
      }

      return resolveHOC((0, _resolveToValue.default)(inner, importer), importer);
    }
  }
