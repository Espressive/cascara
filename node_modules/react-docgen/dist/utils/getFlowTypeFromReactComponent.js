...
var _default = (path, importer) => {
  let typePath = null;

  if ((0, _isReactComponentClass.default)(path, importer)) {
    ...
    if (superTypes.value) {
      ...
      const propsMemberPath = (0, _getMemberValuePath.default)(path, 'props', importer);
...
function applyToFlowTypeProperties(documentation, path, callback, typeParams, importer) {
  if (path.node.properties) {
    ...
    path.get('types').each(typesPath => applyToFlowTypeProperties(documentation, typesPath, callback, typeParams, importer));
    ...
    const typePath = (0, _resolveGenericTypeAnnotation.default)(path, importer);
...
function applyExtends(documentation, path, callback, typeParams, importer) {
  path.get('extends').each(extendsPath => {
    const resolvedPath = (0, _resolveGenericTypeAnnotation.default)(extendsPath, importer);

    if (resolvedPath) {
      if (resolvedPath.node.typeParameters && extendsPath.node.typeParameters) {
        typeParams = (0, _getTypeParameters.default)(resolvedPath.get('typeParameters'), extendsPath.get('typeParameters'), typeParams, importer);
      }
