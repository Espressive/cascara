...
function handleTSTypeReference(path, typeParams, importer) {
  ...
  const resolvedPath = typeParams && typeParams[type.name] || (0, _resolveToValue.default)(path.get('typeName'), importer);

  if (path.node.typeParameters && resolvedPath.node.typeParameters) {
    typeParams = (0, _getTypeParameters.default)(resolvedPath.get('typeParameters'), path.get('typeParameters'), typeParams, importer);
  }
...
function handleTSTypeLiteral(path, typeParams, importer) {
  ...
  path.get('members').each(param => {
    if (_astTypes.namedTypes.TSPropertySignature.check(param.node) || _astTypes.namedTypes.TSMethodSignature.check(param.node)) {
      const propName = (0, _getPropertyName.default)(param, importer);
...
function handleTSUnionType(path, typeParams, importer) {
  return {
    name: 'union',
    raw: (0, _printValue.default)(path),
    elements: path.get('types').map(subType => getTSTypeWithResolvedTypes(subType, typeParams, importer))
  };
}
...
function handleTSFunctionType(path, typeParams, importer) {
  ...
  path.get('parameters').each(param => {
    ...
    const arg = {
      ...
      type: typeAnnotation ? getTSTypeWithResolvedTypes(typeAnnotation, typeParams, importer) : undefined
    };
...
function handleTSTypeQuery(path, typeParams, importer) {
  const resolvedPath = (0, _resolveToValue.default)(path.get('exprName'), importer);
...
function handleTSTypeOperator(path, typeParams, importer) {
  ...
  const resolvedPath = (0, _resolveToValue.default)(value, importer);
  ...
  if (resolvedPath && (_astTypes.namedTypes.ObjectExpression.check(resolvedPath.node) || _astTypes.namedTypes.TSTypeLiteral.check(resolvedPath.node))) {
    const keys = (0, _resolveObjectKeysToArray.resolveObjectToNameArray)(resolvedPath, importer, true);
...
function handleTSIndexedAccessType(path, typeParams, importer) {
  ...
  const objectType = getTSTypeWithResolvedTypes(path.get('objectType'), typeParams, importer); // eslint-disable-next-line no-undef

  const indexType = getTSTypeWithResolvedTypes(path.get('indexType'), typeParams, importer); // We only get the signature if the objectType is a type (vs interface)
