...
const postprocessPropTypes = (path, importer) => _astTypes.namedTypes.Function.check(path.node) ? (0, _resolveFunctionDefinitionToReturnValue.default)(path, importer) : path;

const POSTPROCESS_MEMBERS = new Map([['propTypes', postprocessPropTypes]]);
const LOOKUP_METHOD = new Map([[_astTypes.namedTypes.ArrowFunctionExpression.name, _getMemberExpressionValuePath.default], [_astTypes.namedTypes.CallExpression.name, _getMemberExpressionValuePath.default], [_astTypes.namedTypes.FunctionExpression.name, _getMemberExpressionValuePath.default], [_astTypes.namedTypes.FunctionDeclaration.name, _getMemberExpressionValuePath.default], [_astTypes.namedTypes.VariableDeclaration.name, _getMemberExpressionValuePath.default], [_astTypes.namedTypes.ObjectExpression.name, _getPropertyValuePath.default], [_astTypes.namedTypes.ClassDeclaration.name, _getClassMemberValuePath.default], [_astTypes.namedTypes.ClassExpression.name, _getClassMemberValuePath.default]]);
...
function getMemberValuePath(componentDefinition, memberName, importer) {
  ...
  const lookupMethod = LOOKUP_METHOD.get(componentDefinition.node.type) || _getMemberExpressionValuePath.default;
