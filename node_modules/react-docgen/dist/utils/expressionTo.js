...
function toArray(path, importer) {
  ...
  while (parts.length > 0) {
    ...
    if (_astTypes.namedTypes.CallExpression.check(node)) {
      ...
      if (node.computed) {
        const resolvedPath = (0, _resolveToValue.default)(path.get('property'), importer);
      ...
      const properties = path.get('properties').map(function (property) {
        return toString(property.get('key'), importer) + ': ' + toString(property.get('value'), importer);
      });
      result.push('{' + properties.join(', ') + '}');
      continue;
    } else if (_astTypes.namedTypes.ArrayExpression.check(node)) {
      result.push('[' + path.get('elements').map(function (el) {
