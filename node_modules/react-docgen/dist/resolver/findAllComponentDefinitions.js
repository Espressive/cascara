 ...
 */
function findAllReactCreateClassCalls(ast, parser, importer) {
  ...
  function statelessVisitor(path) {
    if ((0, _isStatelessComponent.default)(path, importer)) {
      definitions.add(path);
    }
  ...
  (0, _astTypes.visit)(ast, {
    ...
    visitCallExpression: function (path) {
      if ((0, _isReactForwardRefCall.default)(path, importer)) {
        // If the the inner function was previously identified as a component
        // replace it with the parent node
        const inner = (0, _resolveToValue.default)(path.get('arguments', 0), importer);
        definitions.delete(inner);
        definitions.add(path); // Do not traverse into arguments

        return false;
      } else if ((0, _isReactCreateClassCall.default)(path, importer)) {
        const resolvedPath = (0, _resolveToValue.default)(path.get('arguments', 0), importer);
