...
function isMethod(path, importer) {
  const isProbablyMethod = _astTypes.namedTypes.MethodDefinition.check(path.node) && path.node.kind !== 'constructor' || (isPublicClassProperty(path) || _astTypes.namedTypes.Property.check(path.node)) && _astTypes.namedTypes.Function.check((0, _resolveToValue.default)(path.get('value'), importer).node);

  return isProbablyMethod && !(0, _isReactComponentMethod.default)(path, importer);
}
...
function findAssignedMethods(scope, idPath, importer) {
  ...
  (0, _traverse.traverseShallow)(scope.path, {
    visitAssignmentExpression: function (path) {
      ...
      }) && path.scope.lookup(name) === idScope && _astTypes.namedTypes.Function.check((0, _resolveToValue.default)(path.get('right'), importer).node)) {
        results.push(path);
        return false;
      }
...
function componentMethodsHandler(documentation, path, importer) {
  ...
  if ((0, _isReactComponentClass.default)(path, importer)) {
    ...
    methodPaths = path.get('properties').filter(props => isMethod(props, importer)); // Add the statics object properties.

    const statics = (0, _getMemberValuePath.default)(path, 'statics', importer);
  ...
  documentation.set('methods', methodPaths.map(p => (0, _getMethodDocumentation.default)(p, importer)).filter(Boolean));
}