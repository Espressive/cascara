...
function displayNameHandler(documentation, path, importer) {
  let displayNamePath = (0, _getMemberValuePath.default)(path, 'displayName', importer);
  ...
  if (!displayNamePath) {
    ...
    } else if (_astTypes.namedTypes.ArrowFunctionExpression.check(path.node) || _astTypes.namedTypes.FunctionExpression.check(path.node) || (0, _isReactForwardRefCall.default)(path, importer)) {
  ...
  displayNamePath = (0, _resolveToValue.default)(displayNamePath, importer); // If display name is defined as a getter we get a function expression as
  ...
  if (_astTypes.namedTypes.FunctionExpression.check(displayNamePath.node)) {
    displayNamePath = (0, _resolveFunctionDefinitionToReturnValue.default)(displayNamePath, importer);
  }
