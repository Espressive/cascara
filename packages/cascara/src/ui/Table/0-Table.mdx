...
By default, all actions are added to **All** rows, this might not be what you want: Under certain scenarios, some actions might not be available or allowed depending on certain (business-logic) conditions. `resolveRecordActions` is function you can pass to derive if a certain action can be displayed for each row.
...
```js
   ...
   * @paramm {Array[Object]} allActions The array of actions specified in `action.modules`
