...
1. **form.start** when entering in **edit** mode
2. **form.cancel** when exiting the **edit** mode via the **cancel** button
...
const onAction = (action, data) => {
  ...
  switch (action.name) {
    ...
    case 'form.cancel':
      ...
      break;
    case 'form.submit':
      // do something when the user exits edit mode via `submit` button
      break;
  }
};
```
