...
import cosmosFixtures, { displayProps, editingProps } from './DataText.fixture';
...
describe('DataText', () => {
  ...
  describe('editing', () => {
    ...
    test('renders a <input date> by default', () => {
      ...
      expect(input).toHaveValue('my text value example');
    });
  ...
  describe('data types', () => {
    ...
    test("what goes in, must go out - 'at rest' data", async () => {
       ...
       */
      const submittedValue = onAction.mock.calls[0][1].text;
    ...
    test("what goes in, must go out - 'mutated' data", async () => {
      ...
      userEvent.type(textInput, ' rocks', { allAtOnce: true });
