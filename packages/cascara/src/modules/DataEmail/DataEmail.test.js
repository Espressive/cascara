...
describe('DataEmail', () => {
  ...
  describe('data types', () => {
    ...
    test("what goes in, must go out - 'at rest' data", async () => {
       ...
       */
      const submittedValue = onAction.mock.calls[0][1].email;
    ...
    test("what goes in, must go out - 'mutated' data", async () => {
      ...
      // make the form dirty
      const colorInput = screen.getByLabelText('Email');
      fireEvent.input(colorInput, {
        target: { value: 'engineers@espressive.com.mx' },
      });
      ...
      expect(onAction).toBeCalledWith(
        ...
        {
          checkbox: false,
          email: 'engineers@espressive.com.mx',
        }
      );
