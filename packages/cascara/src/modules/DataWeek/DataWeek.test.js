...
import cosmosFixtures, { displayProps, editingProps } from './DataWeek.fixture';
...
describe('DataWeek', () => {
  ...
  describe('display', () => {
    ...
    test('renders a <span> by default', () => {
      ...
      expect(input.classList.contains('Week')).toBe(true);
    });
  });
  ...
  describe('editing', () => {
    ...
    test('change value', () => {
      const newWeek = '2021-W15';
      const input = screen.getByLabelText(editingProps.label);
      fireEvent.change(input, { target: { value: newWeek } });
      expect(input).toHaveValue(newWeek);
    });
  });
  ...
  describe('data types', () => {
    ...
    test("what goes in, must go out - 'at rest' data", async () => {
       ...
       */
      const submittedValue = onAction.mock.calls[0][1].week;
    ...
    test("what goes in, must go out - 'mutated' data", async () => {
      ...
      fireEvent.input(weekInput, { target: { value: '2021-W10' } });
