...
describe('DataDateTime', () => {
  ...
  describe('display', () => {
    ...
    test('renders a <span> by default', () => {
      ...
      expect(input.classList.contains('Month')).toBe(true);
    });
  });
  ...
  describe('data types', () => {
    ...
    test("what goes in, must go out - 'at rest' data", async () => {
       ...
       */
      const submittedValue = onAction.mock.calls[0][1].month;
    ...
    test("what goes in, must go out - 'mutated' data", async () => {
      ...
      fireEvent.input(monthInput, { target: { value: '2018-07' } });
