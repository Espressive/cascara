...
import cosmosFixtures, { displayProps, editingProps } from './DataUrl.fixture';
...
describe('DataUrl', () => {
  ...
  describe('display', () => {
    ...
    test('renders a <span> by default', () => {
      ...
      expect(input.classList.contains('Url')).toBe(true);
    });
  });
  ...
  describe('editing', () => {
    ...
    test('change value', () => {
      const newUrl = 'http://mycompany.com';
      const input = screen.getByLabelText(editingProps.label);
  ...
  describe('data types', () => {
    ...
    test("what goes in, must go out - 'at rest' data", async () => {
       ...
       */
      const submittedValue = onAction.mock.calls[0][1].url;
    ...
    test("what goes in, must go out - 'mutated' data", async () => {
      ...
      fireEvent.input(urlInput, {
        target: { value: 'https://www.espressive.com.mx' },
      });
      ...
      expect(onAction).toBeCalledWith(
        ...
        {
          checkbox: false,
          url: 'https://www.espressive.com.mx',
        }
      );
